...
export default function Productcard() {
  ...
  return (
    ...
    <div className=' sm:flex  md:flex-wrap  gap-15 justify-center items-center mt-10 '>
      ...
      {product.map((prod) => (
        <div className='border-2 broder-gray-500 w-85 md:w-100 h-155 rounded-xl bg-black text-white transition delay-150 duration-300 ease-in-out hover:-translate-y-1 hover:scale-110 hover:border-red-500 '>
          <div className='relative cursor-pointer'  onClick={() => handleMove(prod.id)}>
            <img className='w-100 h-95 rounded-t-xl ' src={prod.image} alt="error" />
            <div className=' absolute bottom-82 left-52 md:left-68 w-28 h-7 text-sm font-bold bg-red-500 text-center pt-1 rounded-full  '>{prod.selling}</div>
          </div>
          <div className='px-5  space-y-2'>
            <div className='text-2xl text-red-500 mt-5'>★★★★★</div>
            <h1 className='font-bold text-xl cursor-pointer' onClick={() => handleMove(prod.id)}>{prod.name}</h1>
